...
# 功能: 自动抓取 IP、分片部署(显示进度)、定时校验、官网拦截测试
...
update_firewall() {
    ...
    for ASN in "${ASNS[@]}"; do
        echo -n "正在从 RADB 提取 AS$ASN 的网段... "
        RAW_IPS=$(whois -h whois.radb.net -- "-i origin AS$ASN" | grep -Eo "([0-9]{1,3}\.){3}[0-9]{1,3}/[0-9]{1,2}" | sort -u)
        ASN_COUNT=$(echo "$RAW_IPS" | wc -l)
        echo -e "\e[34m获取到 $ASN_COUNT 个段\e[0m"
        for ip in $RAW_IPS; do ALL_IPS+=("$ip"); done
    done

    total=${#ALL_IPS[@]}
    [ "$total" -eq 0 ] && { echo "错误：抓取 IP 失败，请检查 whois 工具。"; exit 1; }
    echo -e "\e[32m总计获取到 $total 个 IP 网段。\e[0m"
    ...
    echo "开始下发分片规则至 GCP VPC..."
    for (( i=0; i<total; i+=group_size )); do
        batch=("${ALL_IPS[@]:i:group_size}")
        dest_ranges=$(IFS=,; echo "${batch[*]}")
        
        # 显示绝对进度显示
        echo -ne "进度: 正在设置第 \e[33m$current_chunk\e[0m 条规则 / 总计 \e[36m$total_chunks\e[0m 条... \r"
        
        gcloud compute firewall-rules create "${RULE_PREFIX}-$((current_chunk-1))" \
            --action=DENY --rules=all --direction=EGRESS --priority=1000 \
            --destination-ranges="$dest_ranges" --quiet
        
        ((current_chunk++))
    done
    echo -e "\n\e[32m[SUCCESS] 防火墙规则已同步完成。\e[0m"
}

# --- 2. 核心逻辑：检测拦截状态 ---
check_status() {
    echo -e "\n[步骤 2/2] 正在验证 CDN 官网拦截有效性..."
    echo "---------------------------------------"
    for TARGET in "${TEST_TARGETS[@]}"; do
        echo -n "测试访问 $TARGET ... "
        # 使用 -L 跟随跳转，-I 仅获取请求头，3秒超时
        if curl -I -L -s --connect-timeout 3 "https://$TARGET" > /dev/null; then
...
case "$1" in
    ...
    *)
        ...
        if crontab -l 2>/dev/null | grep -q "$SCRIPT_PATH"; then
            echo "检测到系统已配置自动更新，沿用现有定时配置并执行手动同步..."
            update_firewall
            check_status
        else
            # 只有第一次运行且 Crontab 中无记录时才会执行
            install_cron
            update_firewall
            check_status
            echo -e "\n[恭喜] 首次全功能初始化已完成！"
